...
// Эта строчка говорит Express использовать файлы из папки 'public' как статические
app.use(express.static(path.join(__dirname, 'public')));
// -----------------------------------------


// Хранилище пользователей и видео
const users = [];
...
// --- API Маршруты ---
// (Все ваши маршруты /register, /login, /videos и т.д. остаются здесь)
...
app.post('/register', async (req, res) => {
    try {
        ...
        const newUser = { id: users.length + 1, username, password: hashedPassword, purchases: [] };
        users.push(newUser);
...
// Этот маршрут должен быть ПОСЛЕДНИМ. Он отдает index.html для любого запроса,
// который не совпал с API маршрутами или статическими файлами выше.
app.get('*', (req, res) => {
